...
check_containers() {
    local nginx_status=$(docker ps --filter "name=trungfood-nginx-1" --format "{{.Status}}" | grep -c "Up" || echo "0")
    local gateway_status=$(docker ps --filter "name=trungfood-api-gateway-1" --format "{{.Status}}" | grep -c "Up" || echo "0")
...
if [ "$HTTPS_OK" = false ]; then
    ...
    if docker ps | grep -q "trungfood-nginx-1.*Up"; then
        echo "✅ Nginx container is running!"
        if docker ps | grep -q "trungfood-api-gateway-1.*Up"; then
...
if [ "$HTTPS_OK" = true ]; then
    ...
    echo "🔍 Debug commands:"
    echo "  docker ps | grep trungfood"
    echo "  docker logs trungfood-nginx-1 --tail 20"
    echo "  docker logs trungfood-api-gateway-1 --tail 20"
